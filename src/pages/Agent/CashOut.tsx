...
import { useCashOutMutation } from "@/redux/Wallet/wallet.api";
...
const CashOut = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      const res = await cashOut(cashOutData).unwrap();
      if (res.success) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      setErrorMessage(
        error.data?.message || 
        error.data?.error || 
        "Failed to Cash In money. Please try again."
      );
    }
  };
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-card rounded-lg shadow-lg border p-8 mb-6">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">Cash out funds from users wallet instantly</p>
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormField
                ...
                name="userEmail"
                rules={{
                  required: "Users's email is required",
                  pattern: {
        ...
        <div className="bg-card rounded-lg shadow-md border p-6">
          ...
          <ul className="text-sm text-muted-foreground space-y-2">
            ...
            <li className="flex items-center gap-2">
              <div className="w-2 h-2 bg-primary rounded-full"></div>
              Ensure the user's email is correct
            </li>
          </ul>
        </div>
      </div>
